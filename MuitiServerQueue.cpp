...
double area_num_in_queue, area_server_status, mean_interarrival, arrival_rate,
    mean_service, service_rate, sim_time, time_last_event, time_next_event[3],
    total_of_delays, time_arrival[Q_limit + 1];
...
void report() {
    ...
    for (size_t i = 0; i < arrival_rates.size(); ++i) {
        outfile << arrival_rates[i] << "\t" << avg_waiting_times[i] << "\t"
                << avg_queue_lengths[i] << "\n";
    }

    outfile << "\nAnalytical Results:\n";
    outfile << "Arrival Rate\tAnalytical Waiting Time\tAnalytical Queue "
               "Length\tTraffic Intensity\n";
    for (size_t i = 0; i < arrival_rates.size(); ++i) {
        outfile << arrival_rates[i] << "\t" << analytical_waiting_times[i]
...
int main() {
    ...
    for (double arrival_rate = 1; arrival_rate < 100; arrival_rate += 1) {
        ...
        double mean_number_of_customer_in_system =
            traffic_intensity / (1 - traffic_intensity);
        double mean_response_time =
            mean_number_of_customer_in_system / arrival_rate;
        double mean_waiting_time =
            traffic_intensity / ((1 / mean_service) * (1 - traffic_intensity));
        double mean_queue_length =
            traffic_intensity * traffic_intensity / (1 - traffic_intensity);
        analytical_waiting_times.push_back(mean_waiting_time);
        analytical_queue_lengths.push_back(mean_queue_length);
        traffic_intensities.push_back(traffic_intensity);
    }
    report();
    // plot
    std::string title1 = "Average Waiting Time vs Arrival Rate (" +
                         std::to_string(num_delays_required) + " customers)";
    plt::named_plot("Simulation", arrival_rates, avg_waiting_times);
    plt::named_plot("Analytical", arrival_rates, analytical_waiting_times);
    plt::xlabel("Arrival Rate");
    plt::ylabel("Average Waiting Time");
    plt::title(title1);
    plt::annotate(
        "Number of customers simulated: " + std::to_string(num_delays_required),
        0.05, 0.05);
    plt::legend();
    std::string filename1 = "results/avg_waiting_time_" +
                            std::to_string(num_delays_required) + ".png";
    plt::save(filename1);
    plt::show();

    plt::clf();

    std::string title2 = "Average Queue Length vs Arrival Rate (" +
                         std::to_string(num_delays_required) + " customers)";
    ...
    plt::annotate(
        ...
        0.05, 0.05);
    plt::legend();
    std::string filename2 = "results/avg_queue_length_" +
                            std::to_string(num_delays_required) + ".png";
    plt::save(filename2);
    plt::show();
