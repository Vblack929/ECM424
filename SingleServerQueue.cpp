...
int main() {
    ...
    for (double arrival_rate = 1; arrival_rate < 100; arrival_rate += 1) {
        ...
        double mean_number_of_customer_in_system = traffic_intensity / (1 - traffic_intensity);
        double mean_response_time = mean_number_of_customer_in_system / arrival_rate;
        double mean_waiting_time = traffic_intensity / ((1 / mean_service) * (1 - traffic_intensity));
        double mean_queue_length = traffic_intensity * traffic_intensity / (1 - traffic_intensity);
    ...
    report();
    // plot
    std::string title1 = "Average Waiting Time vs Arrival Rate (" + std::to_string(num_delays_required) + " customers)";
    ...
    plt::legend();
    std::string filename1 = "results/avg_waiting_time_" + std::to_string(num_delays_required) + ".png";
    plt::save(filename1);
    plt::show();
    ...
    plt::annotate("Number of customers simulated: " + std::to_string(num_delays_required), 0.05, 0.05);
    plt::legend();
    std::string filename2 = "results/avg_queue_length_" + std::to_string(num_delays_required) + ".png";
    plt::save(filename2);
    plt::show();
